  ...
  var handleFlipCard = function (event) {
    ...
    if (status.code == 3 ) {
      setTimeout(function () {
        ...
        //var score = $(".timer")[0].children[0].textContent;
        
        var timeStr = document.getElementsByClassName("timer")[0].children[0].textContent.split(' ');
        var time = timeStr[2].split(':');
        var min = parseInt(time[0]);
        var sec = parseInt(time[1]);
        var timeForScore = min + (sec / 100);
      ...
      if (min > 0 || sec > 0) {
          document.getElementById('memory--end-game-score').textContent = 'Score: ' + score + '         Time: ' + timeForScore;
          //document.getElementsByClassName("timer")[0].children[0].textContent = "";
          window.scoreTime.clearTime(timeForScore , score);

      }
      
        else
            document.getElementById('memory--end-game-score').textContent = 'Score: ' + score;
